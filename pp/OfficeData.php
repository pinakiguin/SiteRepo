...
if (WebLib::GetVal($_POST, 'FormToken') !== NULL) {
  if (WebLib::GetVal($_POST, 'FormToken') !== WebLib::GetVal($_SESSION,
    ...
    switch (WebLib::GetVal($_POST, 'CmdAction')) {
      ...
      case 'Save':
        //if ($_SESSION['Msg'] === "") {
        $Query = 'Insert Into `' . MySQL_Pre . 'PP_Offices` '
            . '(`OfficeName`, `DesgOC`, `AddrPTS`, `AddrVTM`, '
            . '`PostOffice`, `PSCode`, `SubDivnCode`, `DistCode`, '
            . '`PinCode`, `Status`, `TypeCode`, '
            . '`Phone`, `Fax`, `Mobile`, `EMail`, `Staffs`, `ACNo`,`UserMapID`)'
            . ' Values(\'' . WebLib::GetVal($_SESSION['PostData'], 'OfficeName',
                                            TRUE)
      ...
      case 'Update':
        $Query = 'Update `' . MySQL_Pre . 'PP_Offices` Set '
            . '`OfficeName`=\'' . WebLib::GetVal($_SESSION['PostData'],
                                                 'OfficeName', TRUE) . '\','
            . '`DesgOC`=\'' . WebLib::GetVal($_SESSION['PostData'], 'DesgOC',
                                             TRUE) . '\','
            . '`AddrPTS`=\'' . WebLib::GetVal($_SESSION['PostData'], 'AddrPTS',
                                              TRUE) . '\','
            . '`AddrVTM`=\'' . WebLib::GetVal($_SESSION['PostData'], 'AddrVTM',
                                              TRUE) . '\', '
            . '`PostOffice`=\'' . WebLib::GetVal($_SESSION['PostData'],
                                                 'PostOffice', TRUE) . '\','
            . '`PSCode`=\'' . WebLib::GetVal($_SESSION['PostData'], 'PSCode',
                                             TRUE) . '\','
            . '`PinCode`=\'' . WebLib::GetVal($_SESSION['PostData'], 'PinCode',
                                              TRUE) . '\', '
            . '`Status`=\'' . WebLib::GetVal($_SESSION['PostData'], 'Status',
                                             TRUE) . '\', '
            . '`TypeCode`=\'' . WebLib::GetVal($_SESSION['PostData'],
                                               'TypeCode', TRUE) . '\', '
            . '`Phone`=\'' . WebLib::GetVal($_SESSION['PostData'], 'Phone', TRUE) . '\', '
            . '`Fax`=\'' . WebLib::GetVal($_SESSION['PostData'], 'Fax', TRUE) . '\', '
            . '`Mobile`=\'' . WebLib::GetVal($_SESSION['PostData'], 'Mobile',
                                             TRUE) . '\', '
            . '`EMail`=\'' . WebLib::GetVal($_SESSION['PostData'], 'EMail', TRUE) . '\', '
            . '`Staffs`=\'' . WebLib::GetVal($_SESSION['PostData'], 'Staffs',
                                             TRUE) . '\', '
    ...
    if ($Query !== '') {
      ...
      if ($Inserted > 0) {
        $_SESSION['Msg']      = 'Office ' . WebLib::GetVal($_POST, 'CmdAction')
            . 'd Successfully!';
        $_SESSION['PostData'] = array('CmdAction' => 'Save');
      } else {
        $_SESSION['Msg'] = 'Unable to ' . WebLib::GetVal($_POST, 'CmdAction') . '!';
      }
    }
  }
}

...
if ((WebLib::GetVal($_POST, 'UserID') !== NULL) && (WebLib::GetVal($_POST,
  ...
  $QueryLogin = "Select UserMapID,UserName from `" . MySQL_Pre . "Users` "
      ...
      . " AND MD5(concat(`UserPass`,MD5('" . WebLib::GetVal($_SESSION, 'Token',
                                                            TRUE) . "')))='" . WebLib::GetVal($_POST,
                                                                                              'UserPass',
                                                                                              TRUE) . "'"
      . " AND Activated";
  ...
  if ($rows > 0) {
    ...
    $Data->do_ins_query(
        ...
        . $Data->SqlSafe($_SERVER['HTTP_REFERER']) . "','"
        . $Data->SqlSafe($_SERVER['HTTP_USER_AGENT']) . "','" . WebLib::GetVal($_SESSION,
                                                                               'UserMapID',
                                                                               TRUE) . "','"
    ...
    $Data->do_ins_query(
        ...
        . $Data->SqlSafe($_SERVER['REMOTE_ADDR']) . "','"
        . $Data->SqlSafe($_SERVER['HTTP_REFERER']) . "','"
        . $Data->SqlSafe($_SERVER['HTTP_USER_AGENT']) . "','" . WebLib::GetVal($_POST,
                                                                               'UserID',
                                                                               TRUE) . "','"
        . $Data->SqlSafe($_SERVER['PHP_SELF']) . "','Login: Failed[" . WebLib::GetVal($_POST,
                                                                                      'UserID',
                                                                                      TRUE) . "]','"
        . $Data->SqlSafe($_SERVER['REQUEST_METHOD']) . "','"
        . $Data->SqlSafe($_SERVER['REQUEST_URI'] . $_SERVER['QUERY_STRING']) . "',"
